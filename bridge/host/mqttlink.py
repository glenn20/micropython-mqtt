...
class MQTTlink:
    ...
    def __init__(self, channel, *args, **kwargs):
        ...
        d = defaults  # d is the config dict: initiallise with default values
        for arg in args:
            d.update(arg)
        d.update(kwargs)

        self._host      = d['mqttproxy_host']
        self.user_start = d['user_start']
        self.s_han      = d['status_handler']
    ...
    async def publish(self, topic, msg, retain=False, qos=0):
        ...
        async with self.publock:
            self.channel.send(self._host, argformat(PUBLISH, topic, msg, int(retain), qos))
            # Wait for proxy host to complete. Quick if qos==0. May be very slow
            # in an outage
    ...
    # Command handled directly by mqtt.py on proxy host e.g. MEM
    ...
    async def start(self, channel):
        ...
        if self.lw_topic is not None:
            channel.send(self._host, argformat(WILL, self.lw_topic, self.lw_msg, self.lw_retain, self.lw_qos))
        ...
        channel.send(self._host, argformat(STATUS))
